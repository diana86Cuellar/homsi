...
<?php
	...
	$gdImage = imagecreatefrompng('../img/logo_empresa1.png');//Logotipo
	
	//Objeto de PHPExcel
	$objPHPExcel  = new PHPExcel();
	
	//Propiedades de Documento
	$objPHPExcel->getProperties()->setCreator("Grupo Homsi")->setDescription("Reporte");
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A6', 'IDENTIFICACION');
	...
	while($rows = $resultado->fetch_assoc()){
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$fila, $rows['ID_Eempleado']);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $rows['Emp_Nombre']);
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$fila, $rows['Emp_Apellido']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, $rows['Emp_Correo']);
		$objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, $rows['Emp_telefono']);
	...
	$title->setCaption('Reporte De Empleados');
	
	// definir posicionado gráfico y título
	$chart->setTopLeftPosition('A'.$filaGrafica);
	$filaFinal = $filaGrafica + 12;
	...
	header('Content-Disposition: attachment;filename="ReporteHomsi.xlsx"');
